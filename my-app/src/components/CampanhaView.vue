<template>
        ...
        <v-sheet class="mx-auto" width="300">
          <v-form class="form" v-model="valid">
              ...
              <v-select
                  for="campaign"
                  id="campaign"
                  label="Escolha sim se for iniciar uma campanha"
                  v-model="campanha"
                  :items="['Sim', 'Não']"
                  variant="solo-filled"
                  required
  ...
  async function create() {
    ...
    const { error } = await supabase.from('tabela1').insert({ campanha: campanha.value, name: name.value, description: description.value, photo_url: publicUrl.value, genero: genero.value, recompensa: reward, situation: situation.value, specie: specie.value, city: city.value })
    if (!error) {
      router.push('/contato_campanha');
    }
  }
