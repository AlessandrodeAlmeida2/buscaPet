...
async function seeUser() { 
  const { data, error } = await supabase.from('tabela2').select('user_id').eq('id', getId)
...
onMounted(async () => {
  const { data } = await supabase.from('tabela2').select().eq('id', getId)
    ...
    setTimeout(() => {
      if (latitude.value && longitude.value) {
        ...
        L.marker([latitude.value, longitude.value]).addTo(map)
          .bindPopup('Localização do animal')
          .openPopup()
